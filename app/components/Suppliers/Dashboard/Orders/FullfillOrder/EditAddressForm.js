...
const EditAddressForm = ({ intl, onSubmit, address, handleDelete }) => {
  ...
  function validateInputs() {
    ...
    if (input.addressLine1 === '' || input.city === ''  || input.zipCode === ''  || input.state === '') {
  ...
  const handleFoundAddress = (place) => {
    ...
    const city = ComponentsByType.locality ? `${ComponentsByType.locality.long_name  }` : `${ComponentsByType.administrative_area_level_2.long_name}`
    setInput({
      ...input,
      addressLine1: `${ComponentsByType.street_number.short_name} ${  ComponentsByType.route.short_name}`,
      city: `${city}`,
  ...
  if (Object.keys(address).length !== 0) {  
    deleteButton = <DangerButton onClick={() => handleDelete()}>
      {intl.formatMessage(messages.deleteRecord)}
    </DangerButton> 
  }

  const AddressfromEdit = Object.keys(address).length > 0 ? `${address.addressLine1 } ${ address.city} ${address.state}` : 'Address'
