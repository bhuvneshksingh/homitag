...
import { postModify } from '../../../../containers/Suppliers/Dashboard/Orders/api'
import EditAddressForm from '../Orders/FullfillOrder/EditAddressForm'
...
const AddAddress = ({
  ...
  const handleDelete = () => {
    ...
    const body = product.DeliveryMethods[0]
    body.DeliveryMethodPerPost.customProperties.returnAddresses.splice(
      addressIndex,
      1
    )
  ...
  const onSubmitAddress = addressFromForm => {
    ...
    if (addressFromForm.defaultAddress) {
      if (body.DeliveryMethodPerPost.customProperties.returnAddresses) {
        body.DeliveryMethodPerPost.customProperties.returnAddresses.forEach(
          (element, index) => {
    ...
    if (isNewAddress) {
      if (body.DeliveryMethodPerPost.customProperties.returnAddresses) {
        body.DeliveryMethodPerPost.customProperties.returnAddresses.push(
          addressFromForm
        )
      } else {
        body.DeliveryMethodPerPost.customProperties.returnAddresses = []
    ...
    postModify(product.id, bodySend)
      .then(result => {
        if (result.status === 200) {
          // showConfirmationModal(true)
          // console.log('OK!')
  ...
  return (
    <AddAdressWrapper>
      <StyledTitle>{isNewAddress ? 'Add Address' : 'Edit Address'}</StyledTitle>
