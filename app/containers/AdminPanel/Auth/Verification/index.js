...
import { makeSelectUserInfo } from 'containers/AdminPanel/Auth/selectors';
import Form from 'components/Suppliers/Forms/OnboardingForm';
import Routes from 'containers/AdminPanel/Router/Routes.json';
import { authCheckAction } from 'containers/AdminPanel/Auth/action';
...
const Register = ({ intl, userInfo, history, authCheck }) => {
  ...
  const sendCode = () => {
    ...
    sendCodeService(userInfo.id, 'email', { email: userInfo.email })
      .then(() => setDescription(intl.formatMessage(messages.description, { email: userInfo.email })))
  ...
  const handleSubmit = values => {
    ...
    verificationService(userInfo.id, 'email', { token })
      .then(() => {
        ...
        history.push(Routes.AdminPanel + Routes.Dashboard);
      })
      .catch(e => setError(e.response.data.result.content.message))
  ...
  return (
    <AuthWrapper>
      ...
      <ButtonsWrapper>
        <OutlinePrimaryButton style={{ marginRight: '12px' }} onClick={resendCode} disabled={sendCodeLoading}>
          {intl.formatMessage(messages.cantFindCode)}
        </OutlinePrimaryButton>
        <OutlinePrimaryButton style={{ marginLeft: '12px' }} onClick={showMobileForm}>
