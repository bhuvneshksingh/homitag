...
const DeclineReturn = ({ match, history }) => {
  ...
  let DeclineReturnForm = ''

  if (loading) return <Loading pageLoading transparent size={60} />
  if(order && order.id){

    DeclineReturnForm = 
      <DeclineForm buttonNext={{label: ""}} handleSuccess={handleSuccess} handleError={handleError} handleDecline={decline}/>
    ...
    return ( 
      <Fragment>
        ...
        <StyledTitle>Decline Return</StyledTitle>
...
DeclineReturn.propTypes = {
  match: object,
  history: object
}
export default injectIntl(DeclineReturn)
