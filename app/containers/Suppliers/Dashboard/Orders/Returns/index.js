...
import Filters from 'components/Suppliers/Dashboard/Orders/Returns/Filters'
...
const Returns = () => {
  ...
  const getList = (wasShipped,  isArchived) => {
    ...
    getReturnsList({ wasShipped, searchText, sort: sortAndFilter.sort,  isArchived, returnStatus: sortAndFilter.status, sellerId: cookies.get('userId')  })
      .then(res => {
        if (wasShipped) {
          setShippedTotal({
  ...
  return (
    <>
      <PageHeader pageTitle="Returns" />
      <TabSection/>
      <TopSection container spacing={1}>
        <Grid item xs={10}>
          <SearchBox onChange={value => setSearchText(value)}>Search Return ...</SearchBox>
        </Grid>
...
export default injectIntl(Returns)
